...
export function heartbeatScript(token, healthPath, intervalMs) {
  return '<script async>(function(){var last=' + JSON.stringify(token) +
    ...
    ',{cache:"no-store"}).then(function(r){return r.json()}).then(function(j){if(j&&j.token&&j.token!==last){location.reload()}}).catch(function(){})}chk();setInterval(chk,' +
    intervalMs + ')})();</script>'
}
...
export function apply(ctx) {
  ...
  function handleTrigger(reason) {
    ...
    const st = readState()
    if (st === null) {
      console.warn('[dsh-phoenix] no lifecycle state file (set DSH_PHOENIX_STATE_FILE); durable lifecycle disabled')
      return
    }
  ...
  function recover() {
    const st = readState()
    if (st === null) {
      console.warn('[dsh-phoenix] no lifecycle state file; recovery skipped')
      return
    }
    ...
    if (!target) {
      const ns = afterResume(boot, { now: Date.now() })
      writeState(ns)
      console.warn('[dsh-phoenix] no matching goal found (stale/missing checkpoint?); cleared pending, no resume')
      return
    }
    const goalsSvc = ctx.get('goals')
