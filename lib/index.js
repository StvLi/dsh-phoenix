...
export function apply(ctx) {
  ...
  function startDeferLoop(initial, reason) {
    ...
    const timer = ctx.interval(() => {
      ...
      const decision = deferDecision(cur, {
        now: Date.now(),
        softDeadline: cfg.deferSoftMs,
        hardDeadline: cfg.deferHardMs,
  ...
  // Recovery: on every boot, if we were mid-cycle, move to RECOVERING and resume
  // the recorded goal at-most-once per generation, then settle to RUNNING.
  function findTargetAgent(state) {
    const agentsSvc = ctx.get('agents')
    ...
    for (const a of agentsSvc.list()) {
      ...
      const disarmedActive = g.phase === 'active' && g.activation === 'disarmed'
      if (state.goalId) {
        if (disarmedActive && g.id === state.goalId) return { agent: a, goal: g }
      } else if (disarmedActive && !anyDisarmed) {
        anyDisarmed = { agent: a, goal: g }
      }
    }
    return anyDisarmed
  }
  ...
  function recover() {
    ...
    writeState(boot)
    const dec = resumeDecision(boot, { maxResumeAttempts: cfg.maxResumeAttempts })
    if (dec.action === 'none') {
      const ns = afterResume(boot, { now: Date.now() })
      writeState(ns)
      console.log('[dsh-phoenix] recovered (no resume requested); state=running')
      return
    }
    if (dec.action === 'exhausted') {
      const ns = afterResume(boot, { now: Date.now() })
      writeState(ns)
      console.warn('[dsh-phoenix] resume exhausted (attempts=' + boot.resumeAttempt + '); no further resume (loop prevented)')
      return
    }
    const target = findTargetAgent(boot)
    if (!target) {
      // The goal may not be registered yet; wait a bounded window before calling
      // it stale. resumeAttempt is NOT incremented during these re-scans, so the
      // at-most-once semantics only step in when we actually call resume().
      recoveryFindAttempts += 1
      if (recoveryFindAttempts < cfg.rearmFindAttempts) {
        console.log('[dsh-phoenix] matching goal not registered; re-checking in ' + cfg.rearmRetryMs + 'ms (attempt ' + recoveryFindAttempts + '/' + cfg.rearmFindAttempts + ')')
        ctx.timeout(() => recover(), cfg.rearmRetryMs)
        return
      }
      const ns = afterResume(boot, { now: Date.now() })
      writeState(ns)
      console.warn('[dsh-phoenix] no matching goal after ' + cfg.rearmFindAttempts + ' checks (stale/missing checkpoint?); cleared pending, no resume')
      return
    }
    const goalsSvc = ctx.get('goals')
    ...
    let ok = true
    try {
      goalsSvc.resume(target.agent, { id: target.goal.id, revision: target.goal.revision })
      console.log('[dsh-phoenix] resumed goal ' + target.goal.id + ' (rev ' + target.goal.revision + ', attempt ' + marked.resumeAttempt + ')')
    } catch (e) {
      ok = false
    ...
    writeState(ns)
    console.log('[dsh-phoenix] recovery complete (gen ' + ns.generation + ', state=running, resume=' + (ok ? 'ok' : 'failed') + ')')
  }
  if (cfg.stateFile) {
