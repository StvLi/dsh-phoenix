...
export function apply(ctx) {
  ...
  async function recover() {
    ...
    // Resume the AGENT SESSION (the task) that owns the goal, so the round that
    // was running is re-driven rather than just re-arming the goal marker.
    if (ok) {
      const taskOk = await resumeAgentTask(info.agent.id)
      console.log('[dsh-phoenix] task resume=' + (taskOk ? 'ok' : 'skipped'))
    }
    // Keep goalId + pendingResume: the marked goal is re-resumed after the NEXT restart too.
